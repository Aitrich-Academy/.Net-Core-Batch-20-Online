...
namespace Domain.Service.JobProvider
{
    public class JobProviderService : IJobProviderService
    {
        ...
        public JobProviderService(IJobProviderRepository repository, HireMeNowDbContext context, IMapper mapper)
        {
        ...
        public async Task<List<JobSeekerDto>> GetJobSeekersAsync()
        {
            var jobSeekers = await _repository.GetJobSeekersAsync();
            return _mapper.Map<List<JobSeekerDto>>(jobSeekers);
        }

        public async Task<JobSeekerDto?> GetJobSeekerByIdAsync(Guid id)
        {
            var jobSeeker = await _repository.GetJobSeekerByIdAsync(id);
            return _mapper.Map<JobSeekerDto?>(jobSeeker);
        }

        public async Task<List<JobSeekerDto>> GetJobSeekersByTitleAsync(string title)
        {
            var jobSeekers = await _repository.GetJobSeekersByTitleAsync(title);
            return _mapper.Map<List<JobSeekerDto>>(jobSeekers);
        }
